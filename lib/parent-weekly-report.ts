...
export async function buildWeeklySummary(
  ...
  const [recitationRows, sessionsRows, badgesRows, memPath, tajPath] = await Promise.all([
    ...
    queryOne<{ title: string; progress_percentage: number; total_courses: number }>(
  ...
  if (memPath) {
    level = `${memPath.title} (${Math.round(memPath.progress_percentage)}%)`
  } else if (tajPath) {
    level = `${tajPath.title} (${Math.round(tajPath.progress_percentage)}%)`
  }
