...
export async function GET(
  ...
  // We use a single `learning_paths` table partitioned by `subject`.
  const memorizationPaths = await query<{ id: string; title: string; level: string | null }>(
    `SELECT id::text, title, level FROM learning_paths
     WHERE is_published = true AND subject = 'quran'
     ORDER BY title`
  )

  const tajweedPaths = await query<{ id: string; title: string; level: string | null }>(
    `SELECT id::text, title, level FROM learning_paths
     WHERE is_published = true AND subject = 'tajweed'
     ORDER BY title`
  )
