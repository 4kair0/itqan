...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const earners = await query(
      `SELECT b.awarded_at, b.points_awarded, u.id as user_id, u.name, u.email
       FROM badges b
       JOIN users u ON u.id = b.user_id
       WHERE b.badge_key = $1
       ORDER BY b.awarded_at DESC`,
      [(badge as any).badge_key],
    )

    return NextResponse.json({ data: badge, earners })
  } catch (error) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await query(`
      ...
      RETURNING *
    `, [
      body.badge_name ?? null,
      body.badge_description ?? null,
      body.badge_icon ?? null,
      body.badge_image_url ?? null,
      body.badge_color ?? null,
      body.points_awarded ?? null,
