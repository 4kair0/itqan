...
export default function AdminBadgesPage() {
  ...
  const fetchBadges = async () => {
    try {
      const res = await fetch('/api/academy/admin/badges?stats=true')
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id)
    try {
      await fetch(`/api/academy/admin/badges/${id}`, { method: 'DELETE' })
      fetchBadges()
    } finally {
      setDeletingId(null)
    }
  ...
  const handleToggleActive = async (b: BadgeDef) => {
    await fetch(`/api/academy/admin/badges/${b.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const searchStudents = async () => {
    ...
    try {
      const res = await fetch(`/api/academy/admin/points?search=${encodeURIComponent(awardSearch)}`)
      if (res.ok) {
        const data = await res.json()
