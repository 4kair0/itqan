...
function ParentMessagesContent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const res = await fetch(`/api/academy/parent/children/${newChildId}/teachers`)
      const data = await res.json()
      if (res.ok) {
        setTeachers(data.teachers || [])
        if (data.teachers?.[0]) setNewTeacherId(data.teachers[0].id)
      }
    }
    load()
  }, [newChildId])
  ...
  const startConversation = async () => {
    if (!newChildId || !newTeacherId || !newContent.trim()) return
    setCreating(true)
    try {
      const res = await fetch('/api/academy/parent/conversations', {
        method: 'POST',
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-3xl font-black">
            {isAr ? 'مراسلة المعلمين' : 'Message Teachers'}
          </h1>
        </div>
      ...
      {showNew && (
        <Card className="rounded-2xl">
          <CardContent className="p-6 space-y-3">
            ...
            <div>
              <label className="text-sm font-bold mb-1 block">{isAr ? 'المعلم' : 'Teacher'}</label>
              {teachers.length === 0 ? (
                <div className="text-sm text-muted-foreground">
                  {isAr
                    ? 'لا يوجد معلمون مسجل عندهم الابن حالياً.'
                    : 'No teachers currently teaching this child.'}
                </div>
              ) : (
                <select
                  value={newTeacherId}
            ...
            <Button
              ...
              disabled={creating || !newTeacherId || !newContent.trim()}
              className="w-full"
            >
